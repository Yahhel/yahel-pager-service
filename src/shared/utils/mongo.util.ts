...
export const withTransaction = async (config: SessionConfig) => {
  ...
  const isOffSession = process.env?.OFF_SESSION === 'true';
  const noSession = isLocal || isOffSession;
      ...
      sessionConfigs.map(async (sessionConfig: SessionConfig) => {
        ...
        await childSession.withTransaction(async () => {
          const result = await action?.(
            store,
            noSession ? undefined : childSession,
          );
          store.push(result);
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const result = await action?.([], noSession ? undefined : session);
